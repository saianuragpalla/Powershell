...
if(Test-Connection -ComputerName $node -BufferSize 32 -Count 2 -Quiet)
{
 Get-Service CitrixBrokerService -ComputerName $node | ft machinename,name,displayname, status,starttype -AutoSize 
...
foreach($node1 in $accessedservers)
{
    ...
    Start-Service -InputObject $(Get-Service CitrixBrokerService -ComputerName $node1)

    Write-host "successfully restarted broker services on $node1" -ForegroundColor Green

    Get-Service CitrixBrokerService -ComputerName $node1 | ft machinename,name,displayname, status,starttype -AutoSize 
...
$FinalResult= Get-Service CitrixBrokerService -ComputerName $node2
